...
**Q. 실습 과정 또는 반복문 중, 멈춘것 같고 다음셀도 실행되지 않거나 출력이 중단되지 않을 경우 어떻게 하나요?**

A. 아마도 셀 왼쪽에 **`In [*]`** 이렇게 **`별표`** 표시가 계속 있을겁니다.    
이건 **`계속 실행중`** 이란 뜻이고 종료가 되면 **`In [특정숫자(누적 실행 횟수)]`** 로 변경됩니다.    
따라서 셀을 강제 종료시켜야 다음 셀도 이어서 실행이 가능하고 출력도 중지됩니다.    
