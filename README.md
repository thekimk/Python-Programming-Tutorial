...
- **[수업특징]:** 
> - 그동안 배우고 살아왔던 방식과 다르기에 `어렵고 익숙하지 않고 만만하지 않을 것`
> - 실제 초등/중등학교에서의 강의내용과 다르지 않지만, `책보단 사회적 변화 위주의 내용`으로 `이론이 별로 없음`
> - 컴퓨터와 데이터라는 개념을 처음으로 사용하게 될 것이기 때문에, 익숙하지 않아 `상대적으로 자세하지도 친절하지도 않을 수 있음`
> - 정상적인 컴퓨터로 수업을 진행하는데 이슈는 없지만, 컴퓨팅 환경이 이상하거나 컴퓨터 다루는데 익숙하지 않으면 `예상하진 못한 어려움 존재`
> - `강의시간 이상의 스스로 학습하는 시간`을 들여야 할 것이며, `뒤로 갈수록` 창의성을 요구하기에 `더 어렵게 느껴질 것` 
> - `단!` 본 강의를 끝까지 완주한다면, 그동안 보유하지 못했던 새로운 `경쟁력, 역량 또는 잠재력`을 발견할 수 있으며 `세상 변화를 즐겁게 맞이할 수 있을 것!`
